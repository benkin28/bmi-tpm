...
class nnPANIS(nn.Module): # PANIS
    ...
    def forward_fromX(self, X):
        """
        Forward pass starting directly from the PDE coefficient vector X.
        That is, X is already the coarse-scale or CG input for the PDE solver.
        """
        # 1) Solve PDE with X
        #    'c_x=X' means 'X' has shape [batch_size, self.pde.NofShFuncs] or similar
        # X=X.reshape(100,17,17)
        # print("X",X.shape())
        X=X.reshape(X.size(0), self.reducedDim, self.reducedDim)
        out = self.pde.shapeFunc.solveCGPDE(c_x=X, f=100., uBc=self.uBc)

        # 2) Optional: add random noise if self.yFMode is True
        if self.yFMode:
            out = out.view(-1, self.reducedDim**2) + (
                torch.einsum('ij,...j->...i', torch.pow(10, self.V), torch.randn(out.size(0), self.V.size(-1))) +
                torch.pow(10, self.globalSigma/2.0) * torch.randn(out.size(0), self.reducedDim**2)
            )

        # 3) Map PDE solution to the final shape (same as your original Ytoy)
        out = self.Ytoy(out)

        # Now shape is [batch_size, self.pde.NofShFuncs], reshape to [batch_size, 1, NofShFuncs]
        out = out.view(-1, 1, self.pde.NofShFuncs)
