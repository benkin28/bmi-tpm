...
class pdeForm:
    def __init__(self, nele, shapeFuncsDim, mean_px, sigma_px, sigma_r, Nx_samp, createNewCondField, device, post, rhs=None, reducedDim=None, options=None):
        ...
        self.intPoints = options['integrationGrid']  # It needs to be always a multiple of 8 + 1
        ...
        self.shapeFunc = hatFuncs.rbfInterpolation(self.grid, self.gridW, self.gridW2, self.sgrid, 1 / self.dl ** 2,
                                                   1 / self.dlW ** 2, rhs, uBc=options['boundaryCondition'],
                                                   reducedDim=reducedDim, savePath=self.savePath,
                                                   options=self.optionsMod)
        ...
        self.lengthScale = options['lengthScale']  # 0.2 before 07.09.23, 0.1 before 01.02.23
        self.gpSigma = 1.  # 0.35 before 07.09.23

        # Set volume fraction
        volume_fraction_options = {
            'FR50': 0,
            'FR74': 1.3,
            'FR00': -5.,
            'FR10': -1.28,
        ...
        self.fraction = volume_fraction_options.get(options['volumeFraction'], 0)
        ...
        self.phaseHigh = torch.tensor(contrast_ratio_options.get(options['contrastRatio'], 1.))
        ...
        if self.createNewCondField:
            self.gpCov, self.gpEigVals, self.gpEigVecs = self.doGpForInputs(self.gpgrid, self.lengthScale,
                                                                            self.gpSigma,
                                                                            self.numOfInputs)
    ...
    # This is part of 2 in algorithm 1 right?? basically here we get a random sample of our weighting functions?
    ...
    # What exactly is x in this case? 
    def gpExpansionExponentialParallel(self, x):
        ...
        mask = out > (self.fraction * torch.std(out, dim=[-1, -2]) + (torch.mean(out, dim=[-1, -2]))).view(-1, 1, 1, 1) # mask to cut off
        out = torch.where(mask, self.phaseHigh, self.phaseLow)
        return out

    ### Residual Calculation for the ELBO ###
    # Is this just equ 2P??? Is it evaluated over the grid?
    ...
    def createMeanPredictionsParallel(self, y, yMean):
        yPred = self.shapeFunc.cTrialSolutionParallel(y) #create debuggin breakpoints [100 instances, 100 samples, 65, 65 field ]
        yMean = self.shapeFunc.cTrialSolutionParallel(yMean) #[100, 65,65] 
