...
# ----------------------------------------------------------------------
    ...
    # --- 2‑D coordinates & latent‑dim are still handy, keep them -------------
    x_coords = torch.linspace(0, 1, 17, device=device)
    y_coords = torch.linspace(0, 1, 17, device=device)
    latent_dim = 17 * 17            # 289

    # ----------------------------------------------------------------------
    #                           BETA  PRIOR
    # ----------------------------------------------------------------------
    # • α,β  < 1   → “U‑shaped” density, encouraging draws close to {0,1}.
    # • You can keep them fixed *or* make them learnable Pyro parameters.
    alpha0, beta0 = 0.3, 0.3        # tweak to taste (both < 1)

    # ───────────────────────────────────────────────────────────────────────
    # (re‑)define the Bayesian‑inverse model with the new prior
    # ───────────────────────────────────────────────────────────────────────
    class BayesianInverseModel(PyroModule):
        def __init__(self, neuralNet, V, globalSigma):
            ...
            # OPTIONAL: learn α,β instead of fixing them
            # self.alpha = PyroParam(torch.full((latent_dim,), 0.3, device=device),
            #                        constraint=constraints.positive)
            # self.beta  = PyroParam(torch.full((latent_dim,), 0.3, device=device),
            #                        constraint=constraints.positive)
        ...
        def model(self, y_star, mask):
            ...
            # 1)  Beta prior directly on X  (shape = (289,))
            X = pyro.sample(
                "X",
                dist.Beta(alpha0, beta0)
                    .expand([latent_dim])
                    .to_event(1)           # treat all pixels jointly for ELBO
            ).view(-1, 17, 17)             # reshape to (1,17,17)

            # 2)  Physics‑informed likelihood (unchanged)
            p_y = get_py_given_X_distribution(
                X, self.neuralNet, self.V, self.globalSigma
            )

            # 3)  Select only the observed RBF‑coeffs
        ...
        # ------------------------------------------------------------------
        # GUIDE – keep it automatic; it handles the Beta constraint for you
        # ------------------------------------------------------------------
        def guide(self, y_star, mask):
            pass                       # will be populated by Auto* guide
    ...
    for f in fractions:
        ...
        mask = make_uniform_mask(grid_h, grid_w, k, device=device, seed=42)
        masks[label] = mask
    ...
    for frac_label, mask in masks.items():
        ...
        guide = AutoDiagonalNormal(bim.model)    # works with constrained Beta vars
