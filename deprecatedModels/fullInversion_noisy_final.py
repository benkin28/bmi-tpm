...
# ───────────────────────────────────────────────────────────────────────
    ...
    for σ_rel in noise_levels:
        ...
        mask = torch.ones_like(y_noisy, dtype=torch.float32, device=device)  # ← all pixels observed

        for step in range(1, num_svi_steps + 1):
            loss = svi.step(y_noisy, mask)
            if step % 500 == 0 or step == 1:
                print(f"[{σ_rel}] step {step} ELBO={loss:.1f}")

        # 2) draw posterior Z→X
        pred = Predictive(bim.model, guide=guide,
                        num_samples=num_posterior, parallel=False)
        post = pred(y_noisy, mask)
        ...
        accuracy_results[σ_rel] = float(acc)
        ...
        plt.tight_layout()
        plt.savefig(f"gt_vs_prediction_{σ_rel}.png", dpi=150)
        ...
        axs[1].set_title(f"Posterior Mean x — {σ_rel}")
        axs[1].axis('off')
        ...
        plt.tight_layout()
        os.makedirs(f"loop/images/noise/{σ_rel}", exist_ok=True)

        plt.savefig(f"loop/images/noise/{σ_rel}/{i}.png", dpi=150)
        plt.show()
    ...
    json_path = "loop/accuracy_vs_noise_data.json"
    run_record = {
