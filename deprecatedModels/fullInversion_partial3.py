...
# ----------------------------------------------------------------------
    i# ───────────────────────────────────────────────────────────────────────
# ⮕  DROP-IN PATCH: REPLACE your old GP-prior + BayesianInverseModel with this
    # ───────────────────────────────────────────────────────────────────────
    import pyro
    import pyro.distributions as dist
    from torch.distributions import constraints
    ...
    # Keep your grid coords around if you need them elsewhere, but they are unused now:
    # x_coords = torch.linspace(0, 1, 17, device=device)
    # y_coords = torch.linspace(0, 1, 17, device=device)

    latent_dim = 17 * 17   # = 289

    # 1) U-shaped Beta prior parameters (<1 → mass at 0,1)
    alpha0, beta0 = 0.3, 0.3
    ...
    class BayesianInverseModel(PyroModule):
        def __init__(self, neuralNet, V, globalSigma):
            ...
            # ── OPTIONAL: learn α,β rather than fixing them ──
        ...
        def model(self, y_star, mask):
            # --- 1) draw entire 17×17 field X ∈ (0,1) from independent Beta marginals
            X = pyro.sample(
                "X",
                dist.Beta(alpha0, beta0)
                    .expand([latent_dim])
                    .to_event(1)
            ).view(1, 17, 17)   # reshape into (1,17,17)

            # --- 2) physics-informed likelihood (exactly as before)
            p_y = get_py_given_X_distribution(
                X, self.neuralNet, self.V, self.globalSigma
            )

            # flatten to match y_star & mask
        ...
        def guide(self, y_star, mask):
            # empty — will be filled by AutoDiagonalNormal
            pass
    ...
    for frac_label, mask in masks.items():
        ...
        guide = AutoDiagonalNormal(bim.model)    # matches independent Beta prior
